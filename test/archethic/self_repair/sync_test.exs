defmodule Archethic.SelfRepair.SyncTest do
  ...
  describe "last_sync_date/0" do
    ...
    test "should get the last sync date from the db" do
      ...
      stub(MockDB, :get_bootstrap_info, fn "last_sync_time" ->
        last_sync_date
        |> DateTime.to_unix()
        |> Integer.to_string()
      end)
  ...
  test "store_last_sync_date/1 should store the last sync date into the last sync file" do
    last_sync_date = DateTime.utc_now() |> DateTime.add(-60) |> Utils.truncate_datetime()
    last_sync_time = last_sync_date |> DateTime.to_unix() |> Integer.to_string()

    me = self()

    stub(MockDB, :set_bootstrap_info, fn "last_sync_time", time ->
  ...
  describe "load_missed_transactions/2" do
    setup do
      stub(MockDB, :register_stats, fn _, _, _, _ -> :ok end)
      :ok
    end
    ...
    test "should retrieve the missing beacon summaries from the given date" do
      ...
      welcome_node = %Node{
        ...
        authorized?: true,
        authorization_date: DateTime.add(DateTime.utc_now(), -(86_400 * 10))
      }
      ...
      stub(MockDB, :write_transaction, fn ^tx, _ ->
        send(me, :storage)
        :ok
      end)
      ...
      summary = %BeaconSummary{
        ...
        transaction_attestations: [
          %ReplicationAttestation{
            ...
            confirmations:
              Enum.map(elected_storage_nodes, fn node ->
      ...
      assert :ok =
               DateTime.utc_now()
               |> DateTime.add(-86_400)
               |> Sync.load_missed_transactions(P2P.authorized_and_available_nodes())
    ...
    test "load_missed_transactions should postpone attestation if refused" do
      ...
      tx2 =
        TransactionFactory.create_valid_transaction([],
          index: 1,
          timestamp: DateTime.add(DateTime.utc_now(), -59, :minute)
        )
      ...
      summary_time = BeaconSummaryTimer.previous_summary(DateTime.utc_now())
  ...
  describe "process_summary_aggregate/2" do
    ...
    test "should synchronize transactions" do
      ...
      stub(MockDB, :write_transaction, fn ^transfer_tx, _ ->
  ...
  test "should replicate the transactions and their inputs" do
    fixed_time = ~U[2024-12-04 16:00:00Z]
    ...
    node1 = %Node{
      ...
      authorization_date: DateTime.add(fixed_time, -10),
    ...
    node2 = %Node{
      ...
      network_patch: "AAA",
      authorized?: true,
      reward_address: "reward_address_2",
      authorization_date: DateTime.add(fixed_time, -10),
      enrollment_date: fixed_time
    }
    ...
    stub(MockClient, :send_message, fn
      ^node1, %GetTransaction{}, _ -> {:ok, tx}
      ^node1, %GetTransactionInputs{}, _ -> {:ok, transaction_inputs}
    end)

    stub(MockTransactionLedger, :write_inputs, fn ^tx_address, inputs ->
