...
config :archethic, Archethic.Bootstrap.Sync,
  ...
  out_of_sync_date_threshold:
    "ARCHETHIC_BOOTSTRAP_OUT_OF_SYNC_THRESHOLD"
    |> System.get_env("1296000")
    |> String.to_integer()
...
config :archethic,
       ...
       (case(
          "ARCHETHIC_CRYPTO_NODE_KEYSTORE_IMPL"
...
config :archethic, Archethic.Mining.PendingTransactionValidation,
  allowed_node_key_origins:
    "ARCHETHIC_NODE_ALLOWED_KEY_ORIGINS"
    |> System.get_env("tpm;software")
    |> String.upcase()
...
config :archethic, Archethic.P2P.Listener,
  port: "ARCHETHIC_P2P_PORT" |> System.get_env("30002") |> String.to_integer()
...
config :archethic, ArchethicWeb.Endpoint,
  ...
  http: [:inet6, port: "ARCHETHIC_HTTP_PORT" |> System.get_env("40000") |> String.to_integer()],
  url: [host: nil, port: "ARCHETHIC_HTTP_PORT" |> System.get_env("40000") |> String.to_integer()],
  ...
  https: [
    cipher_suite: :strong,
    otp_app: :archethic,
    port: "ARCHETHIC_HTTPS_PORT" |> System.get_env("50000") |> String.to_integer(),
...
config :archethic, :geopatch_update_time, 600
...
config :archethic, :throttle,
  by_ip_high: [
    period: 1000,
    limit: "ARCHETHIC_THROTTLE_IP_HIGH" |> System.get_env("500") |> String.to_integer()
  ],
  by_ip_low: [
    period: 1000,
    limit: "ARCHETHIC_THROTTLE_IP_LOW" |> System.get_env("20") |> String.to_integer()
  ],
  by_ip_and_path: [
    period: 1000,
    limit: "ARCHETHIC_THROTTLE_IP_AND_PATH" |> System.get_env("20") |> String.to_integer()
  ]

# Do not print debug messages in production
config :logger,
  level: "ARCHETHIC_LOGGER_LEVEL" |> System.get_env("info") |> String.to_atom(),
