defmodule Archethic.Checks.NamedSupervisor do
  ...
  defp traverse({:use, _, [{:__aliases__, _, [:Supervisor]} | _]} = ast, acc) do
    {ast, %{acc | supervisor?: true}}
  end

  defp traverse(
         {{:., _, [{:__aliases__, _, [:Supervisor]}, :start_link]}, _, sup_ast} = ast,
         %__MODULE__{supervisor?: true} = acc
       ) do
    case sup_ast do
      [
        {:__MODULE__, _, nil},
