defmodule Archethic.DB.EmbeddedImpl.P2PView do
  ...
  def handle_continue(:load_from_file, %{filepath: filepath} = state) do
  ...
  def handle_call(:get_node_views, _, %{views: views} = state) do
    {:reply, views, state}
  end

  def handle_cast({:set_node_view, nodes_view}, %{filepath: filepath} = state) do
  ...
  defp serialize([view | rest], acc) do
    ...
    avg_availability_int = trunc(avg_availability * 100)
